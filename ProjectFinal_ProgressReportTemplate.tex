...
\item[Person 2] Person 2 wrote code in a file named \texttt{build\_kpt\_dataset.py}, which takes the data collected in \texttt{mast\_frame\_labels.json} and converts it to YOLO pose training data. This data is stored in a folder called \texttt{mast\_kpt\_data}. They also utilized \texttt{label\_frames.py} to label and store data for 200 frames in the 5 minute clip we have been training with. 
\item[Person 3] Person 3 wrote the code for the projection solver. This takes the kpt data and uses optimization to estimate the camera settings and each boat’s real-world position and heading. Person 3 also wrote code in a file named \texttt{solver\_bundle.py} which takes 5 consecutive sailboat video frames and jointly solves them together to estimate a smooth moving camera path plus each boat's changing real-world position and heading over time.
\end{description}

\subsection*{Mentor meetings}
\begin{description}
    \item[Meeting 1] We met with our mentor over zoom on 4/22/2026.
    \item[Meeting 2] We are planning to meet next with our mentor on 4/29/2026.
\end{document}